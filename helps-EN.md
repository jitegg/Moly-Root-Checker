...
- For daily use, it is recommended to keep USB debugging disabled; enable only when needed
- If USB debugging is detected as enabled, you can disable it in Developer Options
- After flashing or using ADB commands, remember to turn USB debugging off

---

## 9. Soter Key Status

- Soter keys depend on the TEE environment; if TEE is abnormal, Soter will not work properly
- If WeChat Pay fails after flashing, try re-flashing the official full ROM package
- On some devices, SoterService may be stripped after flashing; it needs to be restored from the official ROM
- It is recommended to back up Soter-related files (e.g., /data/data/com.tencent.mm/soter) before flashing
